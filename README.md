...
- Multiple rows may reference the same image to label different coordinates or
  descriptions.
...
- Supply 20 labels. The deterministic 16/4 split generated by
  `scripts/prepare_grounding_data.py` is by label row, so rows using the same
  screenshot can appear in both sets.
